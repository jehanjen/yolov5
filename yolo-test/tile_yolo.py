...
def tiler(imnames, source_path, target_path, falsepath, slice_size, ext):
                ...
                for box in boxes:
                    if pol.intersects(box[1]):
                        ...
                        if not imsaved:
                            ...
                            slice_path = os.path.join(target_path, filename.replace(ext, f'_{i}_{j}{ext}'))
                            slice_labels_path = os.path.join(target_path, filename.replace(ext, f'_{i}_{j}.txt'))
                            print(slice_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-size", type=int, default=416, help="Size of a tile. Default: 416")
    ...
    if len(imnames) == 0:
        ...
        raise Exception("Dataset should contain an equal number of images and txt files with labels")
    ...
    tiler(imnames, args.source, args.target, args.falsefolder, args.size, args.ext)